...
from database.services.game_state_service import next_turn, prev_turn, reset_game, get_game_state, count_active_characters
...
@bp.get("/poll")
def api_poll():
    """Lightweight endpoint for tablets to detect state changes without fetching full data."""
    state = get_game_state()
    count = count_active_characters()
    ts = state.last_updated.isoformat() if state.last_updated else None
    return jsonify({
        "last_updated": ts,
