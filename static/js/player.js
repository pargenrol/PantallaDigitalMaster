...
async function updateWhiteboard() {
  try {
    ...
    if (data.state && playerCanvas) {
      // Apply background color from meta before loading objects
      if (data.meta && data.meta.bg) {
        const BG_COLORS = {
          dark:      '#111111',
          parchment: '#f0ead0',
          light:     '#f8f8f4',
        ...
        playerCanvas.backgroundColor = BG_COLORS[data.meta.bg] || '#111111';
      }
      ...
      playerCanvas.loadFromJSON(data.state, () => {
        ...
        // Apply viewport (zoom + pan) from meta, centrado en la pantalla del player
        if (data.meta && data.meta.zoom) {
          const zoom = data.meta.zoom;
          const cx   = data.meta.centerX ?? (playerCanvas.width  / 2);
          const cy   = data.meta.centerY ?? (playerCanvas.height / 2);
          // Centro del canvas del master en coordenadas del documento → centrar en player
          const tx = -cx * zoom + playerCanvas.width  / 2;
          const ty = -cy * zoom + playerCanvas.height / 2;
          playerCanvas.setViewportTransform([zoom, 0, 0, zoom, tx, ty]);
        }
